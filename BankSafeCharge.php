...
class BankSafeCharge extends BankModule implements IBank
{
    ...
    public function initRequest(Creditcards $card, Cardholders $ch, Orders &$order, $auto_charge = false)
    {
        ...
        if (!$this->allow_no3d && !$this->terminal->is3d) {
            return $this->exitError('Non 3D card not allowed', '004');
        }
        if (!$auto_charge) {
            return $this->exitError('Authorise not allowed', '512');
        }
        if (!$this->isValidTerminal()) {
            return $this->exitError('Terminal attributes error', '005');
        }
        if (!in_array($order->currency_id, $this->bank_currencies)) {
            return $this->exitError('Terminal currency ' . $order->currency_id . ' not allowed', '082');
        }
        ...
        if (empty($responce) || (strpos(trim($responce), '{') !== 0)) {
        ...
        if (($responce_json['transactionStatus'] == 'APPROVED') && !$this->threeDv2) {
            $this->bank_unique_id = $responce_json['transactionId'];
            return $this->exitError('No 3d enrolled', '004');
        }

        // success 3d
        if (($responce_json['transactionStatus'] == 'REDIRECT')
            && isset(
                $responce_json['paymentOption'],
                $responce_json['paymentOption']['card'],
                $responce_json['paymentOption']['card']['threeD'],
                $responce_json['paymentOption']['card']['threeD']['acsUrl']
            )
        ) {
            ...
            $this->response['threedsecure']['term_url'] = 'https://services.continentpay.com/defaultTermUrl/';
            $this->response['threedsecure']['method'] = 'POST';
            $this->response['threedsecure']['form3d_html'] = htmlspecialchars($this->make3dForm());
            return true;
        }

        return $this->exitError($responce_json['gwErrorReason'], '005');
    }
    ...
    /**
     * /* Load currency date currency rates
    ...
    private function getBankToken()
    {
        ...
        if (empty($responce) || (strpos($responce, '{') != 0)) {
            return false;
        }
        $responce_json = json_decode($responce, true);
        if (!is_array($responce_json)
    ...
    private function initPayment(Creditcards $card, Cardholders $ch, Orders $order)
    {
        ...
        if (!is_array($responce_json)
            ...
            /* || 	!isset(
                    $responce_json['paymentOption']['card'],
                    $responce_json['paymentOption']['card']['threeD'],
                    $responce_json['paymentOption']['card']['threeD']['v2supported'],
                    $responce_json['paymentOption']['card']['threeD']['methodUrl'],
                    $responce_json['paymentOption']['card']['threeD']['methodPayload']
                    )
            ||	($responce_json['paymentOption']['card']['threeD']['v2supported'] != 'true') */
        ) {
            return false;
        }
